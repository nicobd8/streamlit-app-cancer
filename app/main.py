...
# In real application, you would load the model and scaler from disk
...
def get_scaled_values(input_dict):
    ...
    # Get scaled values from 0 to 1
    # .items() para desempaquetar el diccionario en pares (key, value)
    # En cada iteración, key tomará el nombre de la característica y value tomará el valor correspondiente del diccionario input_dict
    # Primera iteracion: key = "radius_mean", value = 10.0
    # Segunda iteracion: key = "texture_mean", value = 20.0 y así sucesivamente
...
def add_sidebar():
    ...
    # Diccionario para guardar valores de los sliders para hacer el gráfico y la predicción
    input_dict = {}
    ...
    for label, key in slider_labels:
        input_dict[key] = st.sidebar.slider(
            ...
            value=float(data[key].mean()),  # Establecer el valor predeterminado
        )
...
def main():
    ...
    # Input data seleccionada en el sidebar
    input_data = add_sidebar()

    with st.container():
        st.title("Breast Cancer Predictor")
        st.write(
            "This app predicts whether a breast tumor is malignant or benign based on the input features."
        )
